/*
  ...
  19 April 2022     v2    Improved multi-mode multi-band scheduling
...
void loop()
{
  ...
  if (s_GpsOneSecTick )
  {
    ...
    if ((validGPSflag == 1 | validGPSflag == 2)  & suspendUpdateFlag == false)
    {
      /*
        ----------------------------------------------------------------
        Scheduled transmissions begins here
        ----------------------------------------------------------------
...
{
  int SymbolCount;
  if (mode < 5) // if in not in schedule mode check transmit interval
  {
    TXcount++;
