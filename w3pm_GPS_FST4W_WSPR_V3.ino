...
unsigned long XtalFreq = 25000000, tcount = 2, LEDtimer, mult = 0, TXoffset = 1500;
char StartCommand[7] = "$GPGGA", StartCommand2[7] = "$GPRMC", buffer[280] = "", EW, NS;
...
// Load FST4W Gaussian length
unsigned long FST4W_GaussianLength[4]{
  21,   // 120
  56,   // 300
  173,  // 900
  350   // 1800
  ...
  if (s_GpsOneSecTick )
  {
    ...
    if ((validGPSflag == 1 | validGPSflag == 2)  & suspendUpdateFlag == false)
    {
      if (seconds == 0 & TransmitFlag == true)
      {
        ...
        // ---------------------------------------------------------------------
        {
          for (int timeslot; timeslot < 59; timeslot++)
          {
  ...
  if (TransmitMode == 4)
  {
   ...
   while(SymbolCount < 160)
   {
    //Serial.println(SymbolCount);
     ...
     //for (Gaussian_Count = 0; Gaussian_Count < 32; Gaussian_Count++)
    while (Gaussian_Count < 32)
     {
      //Serial.print(Gaussian_count);
      unsigned long timer = millis();
     ...
     while ((millis() - timer) < FST4W_GaussianLength[T_R_period]) { __asm__("nop\n\t");}
     Gaussian_Count++;
     }
   Gaussian_Count = 0;
   SymbolCount++;
